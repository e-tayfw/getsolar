import dspy
from app.modules.opsCoordination.signatures import FormIngestion, EmailIngestion, GenerateQualificationQuestions, QualificationEvaluator
...
class FormQualificationPipeline(dspy.Module):
    ...
    async def aforward(self, **form_inputs):
        ...
        ingestion_result = await self.form_ingestion.acall(**form_inputs)
        ...
        questions_result = await self.generate_questions.acall(
            lead_id=ingestion_result.lead_id,
            context=context
        )
        return ingestion_result.lead_id, questions_result.questions, context
